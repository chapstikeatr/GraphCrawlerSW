...
std::vector<std::vector<std::string>> bfs_parallel(const std::string &start,
  ...
  for (int d = 0; d < depth; d++) {
    ...
    levels.push_back({});            // may reallocate levels
    const auto &current = levels[d]; // safe now
    auto &next = levels[d + 1];      // safe now
    ...
    for (int t = 0; t < threads_to_use; t++) {
      const int extra = (t < rem) ? 1 : 0;
      const int end = begin + base + extra;

      threads.emplace_back([&, begin, end]() {
        // Each thread uses its own CURL easy handle
...
int main(int argc, char *argv[]) {
  ...
  constexpr int MAX_THREADS =
      8; // tune as needed / as required by the assignment
